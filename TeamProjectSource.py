...
def InitialState():
    BUCKET_KEY = "LP6CERJRGM9M"
    ACCESS_KEY = "ist_hC1kDKC28JZ3U8fnnV4sxRlLw6Y8cbVo"
...
class Ultras:
    ...
    """
      class UltraSonics():
        ...
        GPIO.setup(triggerOne, GPIO.OUT)
        GPIO.setup(echoOne, GPIO.IN)
        GPIO.output(triggerOne, False)
        ...
        def distance(self):    
            ...
            return distance
        """
#EVERYTHING AFTER THIS IS THE THUNDERBORG LIBRARY.
...
voltageIn = 12.0 #Added input voltage
voltageOut = (voltageIn * 0.8) #Added output voltage

fPower = 1 #Added power variable for full forwards
rPower = -1 #Added power variable for full reverse
...
If tryOtherBus is True, this function will attempt to use the other bus if the ThunderBorg devices can not be found on the current busNumber
    This is only really useful for early Raspberry Pi models!
        ...
        if not self.foundChip:
            ...
            self.Print("Loaded ThunderBorg sucessfully")
